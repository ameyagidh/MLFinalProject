...
def download_stock(stock):
    try:
        ...
        basePath = '../Data/Raw Data/'
        if "^GSPC":
            output_name = basePath + 'ScrapedSP500.csv'
            stock_df.to_csv(output_name)
        else:
            # os.mkdir(basePath + "stocks")
            # basePath = basePath + "stocks"
            stock_df['Name'] = stock
            output_name = basePath + stock + '_data.csv'
            stock_df.to_csv(output_name)
    except:
...
if __name__ == '__main__':
    print("Program {} is running.".format(argv[0]))
    
    """ set the download window """
    now_time = datetime.now()
    start_time = datetime(now_time.year - 10, now_time.month , now_time.day)

    # S&P500 index vs all tickers in S&P500 
    switch = None
    if len(argv) > 1:
        switch = argv[1]
    ...
    #to keep track of failed queries
    bad_names =[] 
